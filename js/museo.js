/* ==========================================================================
   ...
   Por ahora: el carrusel de apertura. El paneo del cierre es CSS puro
   (animation con alternate), así que no necesita nada de acá.
...
window.initMuseo = function () {
  ...
  /* ------------------------------------------------------------------------
     EL CARRUSEL
     Pedido cerrado: automático Y manual, los dos. Avanza solo cada
     PAUSA milisegundos, y además se puede mover con las flechas, con los
     puntos, con el teclado y con el dedo.

     REGLA IMPORTANTE: apenas el visitante toca algo, el automático se
     apaga para siempre. Si siguiera andando, la imagen se le escaparía
     justo mientras la está mirando, que es la peor forma de perder una
     foto buena.
     ------------------------------------------------------------------------ */

  const PAUSA = 5000;          // ms que queda quieta cada imagen
  const MIN_SWIPE = 45;        // px de arrastre para que cuente como swipe

  const carrusel = document.querySelector("[data-museo-carrusel]");
  if (!carrusel) return;       // no estamos en la página de Museo
  ...
  // Teclado: flechas izquierda y derecha cuando el carrusel tiene el foco.
